...
The client supports initialize/initialized, thread start/resume, turn start/steer/interrupt, correlated completion, item lifecycle/deltas, token usage, approval, permission, user question, and conservative MCP elicitation responses.
...
Fixtures lock the verified Codex `0.147.0` request/notification shapes, including early notifications and cross-thread isolation. A version upgrade should regenerate official protocol bindings for comparison and run the real smoke before updating the compatibility table.
